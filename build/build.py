...
def rpmmain():
    ...
    if not os.path.isfile('%s/nagstamon.spec' % (options.redhat)):
        ...
        return
    execute_script_lines(['cd %(target)s; ln -s %(redhat)s; tar -czf redhat/nagstamon_0.9.12.tar.gz .; fakeroot rpmbuild --define "_sourcedir %(redhat)s" -ba redhat/nagstamon.spec; rm redhat'],
                         get_opt_dict(options))
